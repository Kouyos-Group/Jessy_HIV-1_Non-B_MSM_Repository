...
### Load the data
setwd("~/Desktop/SHCS")
DATA <- read.csv("Output/study_population_update.csv")
#DATA <- read.csv("Output/real_studypopulation_FEB21.csv")
DATA <- DATA[,-1]

VAR_REGION <- read_dta("Input/var_region0620.dta")  #24
ADMI <- read_dta("Input/admi0620.dta") #20,904
TEST_INFO <- read_dta("Input/test_info0620.dta") #24 564
PAT <- read_dta("Input/pat0620.dta") #20 904
LAB <- read_dta("Input/lab0620.dta") #667 245
...
sum(is.na(DD$hiv_posdate)) #1445
sum(is.na(DD$hiv_posdocdate)) #3298
sum(is.na(DD$hiv_negdate)) #7154
sum(is.na(DD$cat)) #12,846
sum(is.na(DD$rule)) #0


sum(is.na(DD$education)) #443
sum(is.na(DD$infect_place)) #8577
8577/12954 #0.6621
...
DD <- DD[DD$subtype != "UNKNOWN" & DD$subtype != "UNDETERMINED (SHORT SEQUENCE)", ] 
DD <- DD[DD$subtype != "HIV 1 O GROUP" & DD$subtype != "HIV 1 N GROUP", ] #12,852
...
lab_noNA <- lab[!is.na(lab$rna),] #need to have a VL value!
# lab$logRNA <- log10(lab$rna)
# lab$logRNA2 <- ifelse(lab$logRNA<1.7,"<1.7",lab$logRNA) #values below 50 might not be reliable
...
agg_ratioRNA$log <- log10(agg_ratioRNA$mean_rna)
agg_ratioRNA$logcorrected <- ifelse(agg_ratioRNA$log<1.7,"<1.7",agg_ratioRNA$log) #values below 50 might not be reliable
names(agg_ratioRNA) <- c("id", "mean_rna","log_mean_rna","log_mean_rna_corrected")
...
############################
### Save the dataset:
#write.csv(DD,"Output/fulltable_study_population_update.csv")
