...
# setwd("~/Desktop")
# Clusters <-  read.csv("MSMcluster_distance15_subtypeAE.csv")[,-c(1)]
# setwd("~/Desktop/SHCS")
setwd("~")
Clusters <- read.csv("Downloads/MSMHET_MTCS_above50/MSMHETcluster_GD45B80_CRF01_AE.csv")[,-c(1)]
setwd("~/Desktop/SHCS")
...
MyTree <- read.tree("Input/AE_tree.tre")
...
SHCStree <- drop.tip(MyTree,AlamosTips)
...
MSMClusters <- Clusters[Clusters$MSMQ == TRUE,] #62
unique(MSMClusters$clusterNR) #12
length(unique(MSMClusters$clusterNR))
MSMClusters$Cl_NR <- ifelse(MSMClusters$clusterNR == 1, 4,
                            ifelse(MSMClusters$clusterNR == 2, 7, 
                                   ifelse(MSMClusters$clusterNR == 3, 9, 
                                          ifelse(MSMClusters$clusterNR == 5, 8, 
                                                 ifelse(MSMClusters$clusterNR == 10, 6, 
                                                        ifelse(MSMClusters$clusterNR == 12, 11,
                                                               ifelse(MSMClusters$clusterNR == 23, 12, 
                                                                      ifelse(MSMClusters$clusterNR == 26, 10, 
                                                                             ifelse(MSMClusters$clusterNR == 36, 3, 
                                                                                    ifelse(MSMClusters$clusterNR == 40, 2, 
                                                                                           ifelse(MSMClusters$clusterNR == 43,1,
                                                                                                  ifelse(MSMClusters$clusterNR == 45, 5,0))))))))))))
# MSMClusters$Cl_NR <- ifelse(MSMClusters$clusterNR == 1, 1,
#                       ifelse(MSMClusters$clusterNR == 2, 2, 
#                              ifelse(MSMClusters$clusterNR == 4, 3, 
#                                     ifelse(MSMClusters$clusterNR == 5, 4, 
#                                            ifelse(MSMClusters$clusterNR == 6, 5, 
#                                                   ifelse(MSMClusters$clusterNR == 7, 6,
#                                                          ifelse(MSMClusters$clusterNR == 11, 7, 
#                                                                 ifelse(MSMClusters$clusterNR == 14, 8, 
#                                                                        ifelse(MSMClusters$clusterNR == 15, 9, 
#                                                                               ifelse(MSMClusters$clusterNR == 17, 10, 
#                                                                                      ifelse(MSMClusters$clusterNR == 22, 11,
#                                                                                             ifelse(MSMClusters$clusterNR == 23, 12, 
#                                                                                                    ifelse(MSMClusters$clusterNR == 26, 13, 
#                                                                                                           ifelse(MSMClusters$clusterNR == 27, 14, 
#                                                                                                                  ifelse(MSMClusters$clusterNR == 28, 15, 
#                                                                                                                         ifelse(MSMClusters$clusterNR == 29, 16,
#                                                                                                                                ifelse(MSMClusters$clusterNR == 42, 17,
#                                                                                                                                       ifelse(MSMClusters$clusterNR == 44, 18,
#                                                                                                                                              ifelse(MSMClusters$clusterNR == 45, 19,0)))))))))))))))))))
...
ncol<-c("darkblue","red", "darkgrey",
        ...
        brewer.pal(8, "Set2"),brewer.pal(4, "Set3"),"white")
...
p <- plotTree %<+% dd +
  ...
  #geom_tiplab(aes(label = paste0(diag_year,"_P_",sample(1:100, nrow(dd), replace=TRUE))),size=1.5,align=TRUE,linetype='dashed', linesize=.1, offset = 0.01) +
  geom_treescale(fontsize=5.5,x=-0.05, y=80) +
  scale_color_manual(breaks =  c(paste0("Cl_",seq(1,12,1))) , values = c( brewer.pal(8, "Set2"),brewer.pal(4, "Set3")))+ 
  no_legend()
...
pfirst<-gheatmap(p,dd[, "riskNEW", drop=F], offset=0.008, width=.15,colnames=FALSE)#+ no_legend()
...
p3x<-pthird+
  scale_fill_manual(breaks =  c(paste0("Cl_",seq(1,12,1))) , values = c( brewer.pal(8, "Set2"),brewer.pal(4, "Set3")))+ 
  guides(color = FALSE)+
  theme(legend.title = element_text(size=20, face = "bold"),legend.title.align=0,legend.text = element_text(size=20))+ 
...
pthirdfirst <- gheatmap(pthird,dd[, "riskNEW", drop=F], offset=0.030, width=.15,colnames=FALSE)+ no_legend()
#scale_fill_manual(values=ncol)+ theme(legend.position="none")
pfirstthirdsecond<- gheatmap(pthirdfirst, dd[, "regionNEW", drop=F], offset=0.045, width=.15,colnames=FALSE)+ no_legend()
...
#   theme(legend.position="none", plot.title = element_text(size = 40, face = "bold"))+
#   ggtitle("Subtype CRF01_AE")


allAE<-pfirstthirdsecond+
  scale_fill_manual(values=ncol)+
...
AEplot<-plot_grid(allAE, leg3, leg1,leg2, ncol=4, rel_widths=c(1.5, .2,.3,.5)) 
AETREE <-plot_grid(allAE,  ncol=4, rel_widths=c(4.5, .2,.3,.5)) 



## Save the plot:
# setwd("~/OneDrive/Master Thesis/Figures/Trees")
# ggsave("CRF01_AE_Tree.tiff", width = 50, height = 50, units = "cm", limitsize = FALSE)
...
MyTree<- read.tree("Input/AE_Tree.tre")
plot(MyTree)
int_nodes <- unique(MyTree$edge[,1]) #829
nr_ext_nodes<- length(MyTree$tip.label) #837
...
## Extracting clades (i.e. given an inner node, we get the tips coming from that node)
getCladeFromNode<-function(x,xTree){ # x is any internal node of xTree
  if(x<=length(xTree$tip.label)) tps<-x # if x is external node
  else {tps<-xTree$edge[,2][xTree$edge[,1]==x]
  tps<-unlist(sapply(tps,function(z) getCladeFromNode(z,xTree))) #if it was not an external node, go deeper inside
  }
  as.character(tps)
}

## All nodes and their depths
nodeDepth <- as.data.frame(node.depth(MyTree))
names(nodeDepth) <- "depth"
nodeDepth$node <- as.numeric(row.names(nodeDepth))
## Remove tips:
nodeDepth <- nodeDepth[nodeDepth$depth != 1,]

#####

## We make a second tree (subtree), were we only consider the SHCS sequences.
LAtips <-  as.numeric(dataTipsandLabel[grepl("[[:alpha:]]", dataTipsandLabel$header),'tip'])
SHCStree <- drop.tip(MyTree,LAtips)
dataTipsandLabel$header <- as.character(dataTipsandLabel$header)
# Function to plot clade with ID and ethNew in it
dataTipsandLabel <- merge(dataTipsandLabel, TestInfo_need, all.x = T)
dataTipsandLabel$id <- ifelse(is.na(dataTipsandLabel$id), "LA", as.character(dataTipsandLabel$id))
dataTipsandLabel <- merge(dataTipsandLabel, PAT[,c('id', 'riskNEW', 'ethNEW')], all.x = T)
...
dataTipsandLabelSHCS$tip <- row.names(dataTipsandLabelSHCS)
names(dataTipsandLabelSHCS) <- c('header','tipSHCS')
## Unfortunately, the tips are newly numbered now, so we have to keep track of that(!)
mergeTips <- merge(dataTipsandLabel, dataTipsandLabelSHCS)

#####
## In DistM, we save the pairwise distance of all SHCS tips. (for whole tree, this would be too large)
DistM <- as.data.frame(cophenetic.phylo(SHCStree))
#####
