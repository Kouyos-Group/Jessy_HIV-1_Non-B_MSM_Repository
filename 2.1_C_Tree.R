...
#setwd("~/Desktop")
#Clusters <-  read.csv("MSMHETcluster_distance15boot80_subtypeG.csv")[,-c(1)]
setwd("~")
Clusters <- read.csv("Downloads/MSMHET_MTCS_above50/MSMHETcluster_GD45B80_C.csv")[,-c(1)]
setwd("~/Desktop/SHCS")
...
MyTree <- read.tree("Input/C_tree.tre")
...
MSMClusters <- Clusters[Clusters$MSMQ == TRUE,] #4
unique(MSMClusters$clusterNR) 
length(unique(MSMClusters$clusterNR)) #1
MSMClusters$Cl_NR <- ifelse(MSMClusters$clusterNR == 4, 1, 0)
...
pthirdfirst <- gheatmap(pthird,dd[, "riskNEW", drop=F], offset=0.040, width=.15,colnames=FALSE)+ no_legend()
#scale_fill_manual(values=ncol)+ theme(legend.position="none")
pfirstthirdsecond<- gheatmap(pthirdfirst, dd[, "regionNEW", drop=F], offset=0.066, width=.15,colnames=FALSE)+ no_legend()
...
Cplot<-plot_grid(allC, leg3, leg1,leg2, ncol=4, rel_widths=c(1.5, .2,.3,.5)) 
CTREE<-plot_grid(allC, ncol=4, rel_widths=c(4.5, .2,.3,.5)) 


## Save the plot:
# setwd("~/OneDrive/Master Thesis/Figures/Trees")
# ggsave("C_Tree.tiff", width = 50, height = 50, units = "cm", limitsize = FALSE)
