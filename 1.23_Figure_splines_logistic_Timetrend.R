...
gl <- grid.layout(nrow=2, 
                  ncol=1)
vp.1 <- viewport(layout.pos.col=1, 
                 layout.pos.row=1) 
vp.2 <- viewport(layout.pos.col=1, 
                 layout.pos.row=2)
...
setEPS()
postscript("whateverMarchMSM.eps")
#MSM:
par(mfrow=c(2,2))
par(cex = 1)
par(mar=c(4.1,4.1,4.1,2.1))
# par(mar=c(3.1,3.1,2.5,1.1))
# par(mar=c(2.1,3.1,2.5,1.1))
...
mtext(side = 2,  text = c("Log-Odds"),
      ...
      line = c(2.5), cex= 1.5)
par(xpd=NA)
text(par("usr")[1]-10, par("usr")[4]+2, "C", cex=1.5, font = 2)
par(cex = 1.8)
legend(par("usr")[2]+10 ,par("usr")[4]+3,
       c("",
         "univariable (Year of Diagnosis)", "logistic regression, df = 1",
         "univariable (Year of Diagnosis)","B-spline regression, df = 4",
         " ",
         "compare to null model","LR > 10, p-value < 0.001 ***",
         "compare to null model","LR > 10 , p-value < 0.001 ***",
...
segments(x0=1981.6,y0=0,x1=2020.4,y1=0,col="black", lwd=2,lty=2 )
par(xpd=NA)
text(par("usr")[1]-10, par("usr")[4]+2, "D", cex=1.5, font = 2)
par(cex = 1.8)
legend(par("usr")[2]+10 ,par("usr")[4]+3,
       c("",
         "CRF01_AE", 
...
setEPS()
postscript("whateverMarchHET.eps")
#HET:
par(mfrow=c(2,2))
par(cex = 1)
par(mar=c(4.1,4.1,4.1,2.1))
...
legend(par("usr")[2]+10 ,par("usr")[4]+3,
       c("",
         ...
         "compare to null model","LR > 10, p-value < 0.001 ***",
         "compare to null model","LR > 10, p-value < 0.001 ***", 
         "compare models","LR > 10, p-value < 0.001 ***"),
