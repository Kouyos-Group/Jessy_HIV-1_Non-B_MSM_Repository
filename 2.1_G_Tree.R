...
setwd("~")
Clusters <- read.csv("Downloads/MSMHET_MTCS_above50/MSMHETcluster_GD45B80_G.csv")[,-c(1)]
setwd("~/Desktop/SHCS")
...
MyTree <- read.tree("Input/G_tree.tre")
...
MSMClusters$Cl_NR <- ifelse(MSMClusters$clusterNR == 2, 1, 0)
...
#scale_fill_manual(values=ncol)+ theme(legend.position="none")
pfirstthirdsecond<- gheatmap(pthirdfirst, dd[, "regionNEW", drop=F], offset=0.065, width=.15,colnames=FALSE)+ no_legend()
...
Gplot<-plot_grid(allG, leg3, leg1,leg2, ncol=4, rel_widths=c(1.5, .2,.3,.5)) 
GTREE<-plot_grid(allG, ncol=4, rel_widths=c(4.5, .2,.3,.5)) 


## Save the plot:
# setwd("~/OneDrive/Master Thesis/Figures/Trees")
# ggsave("G_Tree.tiff", width = 50, height = 50, units = "cm", limitsize = FALSE)
