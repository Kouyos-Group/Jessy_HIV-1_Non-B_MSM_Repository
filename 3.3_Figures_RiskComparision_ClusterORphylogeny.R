...
ALL2$Cluster <- plyr::mapvalues	(ALL2$Cluster, from = c(0,1), to = c("NCL","CL"))
ALL2$Cluster <- factor(ALL2$Cluster, levels = c("NCL","CL"))
...
setwd("~/switchdrive/Master Thesis/Figures/ClustervsRisk")
# setEPS()
# postscript("NEWRiskvsClusterVersionMarch2_panelB.eps")
cotabplot(~ Risk+ Region + Cluster | Subtype, data = table2, 
          split_vertical = TRUE,  #to get the risk above
          labeling_args = list( rot_labels = c( left = 0, bottom=40),
                                offset_varnames = c(left= 0.2),
                                offset_labels = c(left = 0.0)),
          margins = c(left = 0.1),
          gp = gpar(fill =mycol2), col = mycol2,
          spacing = spacing_equal(unit(0.9, "lines")),
...
# setEPS()
# postscript("NEWRiskvsCluster_panelAVersionMarch.eps")
