...
#setwd("~/Desktop")
#Clusters <-  read.csv("MSMHETcluster_distance15boot80_subtypeAG.csv")[,-c(1)]
setwd("~")
Clusters <- read.csv("Downloads/MSMHET_MTCS_above50/MSMHETcluster_GD45B80_CRF02_AG.csv")[,-c(1)]
setwd("~/Desktop/SHCS")
...
MyTree <- read.tree("Input/AG_tree.tre")
...
MSMClusters <- Clusters[Clusters$MSMQ == TRUE,] #28
unique(MSMClusters$clusterNR) 
length(unique(MSMClusters$clusterNR)) #6
MSMClusters$Cl_NR <- ifelse(MSMClusters$clusterNR == 1, 6,
                            ifelse(MSMClusters$clusterNR == 3, 2, 
                                   ifelse(MSMClusters$clusterNR == 11, 4, 
                                          ifelse(MSMClusters$clusterNR == 23, 1, 
                                                 ifelse(MSMClusters$clusterNR == 38, 5, 
                                                        ifelse(MSMClusters$clusterNR == 40, 3,0))))))
...
pthirdfirst <- gheatmap(pthird,dd[, "riskNEW", drop=F], offset=0.032, width=.15,colnames=FALSE)+ no_legend()
#scale_fill_manual(values=ncol)+ theme(legend.position="none")
pfirstthirdsecond<- gheatmap(pthirdfirst, dd[, "regionNEW", drop=F], offset=0.049, width=.15,colnames=FALSE)+ no_legend()
...
AGplot<-plot_grid(allAG, leg3, leg1,leg2, ncol=4, rel_widths=c(1.5, .2,.3,.5)) 
AGTREE<-plot_grid(allAG,  ncol=4, rel_widths=c(4.5, .2,.3,.5)) 



# ## Save the plot:
# setwd("~/OneDrive/Master Thesis/Figures/Trees")
# ggsave("CRF02_AG_Tree.tiff", width = 50, height = 50, units = "cm", limitsize = FALSE)
